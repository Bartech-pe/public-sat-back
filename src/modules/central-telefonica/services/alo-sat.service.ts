...
import { AmiService } from '@modules/call/services/ami.service';
...
export class AloSatService {
  ...
  async getAgent(idUser: number): Promise<UserAgent> {
    const userVici = await this.userVicidialRepository.findOne({
      where: {
        idUser,
      },
    });
  ...
  async findAllCampaigns(idUser: number): Promise<VicidialCampaign[]> {
    /* Obtenemos las credenciales del agente a partir del id del usuario */
    const { agentUser } = await this.getAgent(idUser);
  ...
  async agentLogin(idUser: number, campaign: string): Promise<any> {
    // 1. Obtener credenciales del agente
    const { agentUser, userPass, phoneLogin, phonePass } =
      await this.getAgent(idUser);
  ...
  async agentLogout(idUser: number) {
    /* Obtenemos las credenciales del agente a partir del id del usuario */
    const { agentUser, userPass, phoneLogin } = await this.getAgent(idUser);
  ...
  async resumeAgent(idUser: number): Promise<any> {
    /* Obtenemos las credenciales del agente a partir del id del usuario */
    const { agentUser, userPass, phoneLogin, phonePass } =
      await this.getAgent(idUser);
  ...
  async agentStatus(idUser: number): Promise<any> {
    /* Obtenemos las credenciales del agente a partir del id del usuario */
    const { agentUser, userPass } = await this.getAgent(idUser);
  ...
  async endCall(idUser: number): Promise<any> {
    /* Obtenemos las credenciales del agente a partir del id del usuario */
    const { agentUser, userPass } = await this.getAgent(idUser);
    ...
    if (res) {
      await this.pauseAgent(idUser, VicidialPauseCode.WRAP);
    }
  }
  ...
  async transferCall(idUser: number, idUserTransfer: number): Promise<any> {
    const { agentUser, userPass, phoneLogin } = await this.getAgent(idUser);

    const transferUser = await this.getAgent(idUserTransfer);

...
export class AmiService implements OnModuleInit {
  ...
  constructor(
    ...
    try {
      const port = this.configService.get<number>('AMI_PORT') ?? 5038;
      const host = this.configService.get<string>('AMI_URL') ?? '';
      const user = this.configService.get<string>('AMI_USERNAME') ?? '';
      const pass = this.configService.get<string>('AMI_PASSWORD') ?? '';
  ...
  async InterferCall(body: InterferCallDTO) {
    ...
    const vicidial = await this.userRepository.findAll({
      ...
      where: { idUser: fullUser.id },include: [
              {
                model:User,
                as: 'user',
                attributes:['id'],
    ...
    const replaceAgent = await this.userRepository.findAll({
      attributes: ['username', 'phoneLogin'],//1001
      where: { phoneLogin: replace?.agent },include: [
        {
          model:User,
          as: 'user',
          attributes:['id'],
    ...
    await this.aloSatService.transferCall(replaceAgentJson[0].id,vicidialJson[0].id);
  }

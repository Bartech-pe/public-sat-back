...
import { formatDateTime, formatDuration } from '@common/helpers/time.helper';
...
import {
  ...
  StateCountItems,
  StateCountQuery,
} from '../dto/call/CallCollection';
import { UserRepository } from '@modules/user/repositories/user.repository';
import { InjectConnection } from '@nestjs/sequelize';
import { UserVicidialRepository } from '@modules/user/repositories/user-vicidial.repository';
import { UserVicidial } from '@modules/user/entities/user-vicidial.entity';
import { AdvisorDTO } from '../dto/ami/AMI';
...
export class CallService {
  constructor(
    ...
    private readonly userRepository: UserVicidialRepository,
  ...
  `;
    ...
    SELECT
      ...
      vl.list_id,
      CASE 
        WHEN vcl.status IN ('01', '02', '03', '04', '05', '06', '1', '2', '3', '4', '5', '6', 'SALE', 'A', 'CALLBK', 'INTERESTED', 'CONTACT', 'INFO', 'DISPO') THEN 'Concluido'
        WHEN vcl.status IN ('07', '08', '09', '10', '11', '12', '13', '7', '8', '9', 'DROP', 'HANGUP', 'BUSY', 'NA', 'N', 'MAXCAL', 'ABANDON') THEN 'Abandonado'
        WHEN vcl.status IN ('14', '15', '16', '17', 'QUEUE', 'TIMEOT', 'XFER', 'INCALL', 'CBHOLD') THEN 'Escalado'
        ELSE 'Concluido'
      END as status
    FROM
      recording_log rl
  ...
  `;
    ...
    //console.log("============================ DEBUG ===============================");
    //console.log("SQL WHERE:", where.join(' AND '));
    ...
    const collection: DataCollection<CallItemRow> = {
      ...
      pages: GetPages(filter.limit, total),
    };
  ...
  async findByCategories() {
    const allStates = await this.stateRepository.findAll({
      attributes: ['callStateId', 'name', 'icon', 'style'],
      where: {
        callStateId: {
          [Op.in]: [1, 2, 3],
        },
  ...
  async getAdvisors() {
    ...
    `;
      const results = await this.centralConnection.query<AdvisorItem>(sql, {
        type: QueryTypes.SELECT, replacements: { phoneLogins },
      });
      return results;
    } catch (error) {
      console.log(error);
  ...
  async GetAdvisorsInfo (){
    ...
    const filteredAdvisors: AdvisorItemInfo[] = Advisors
      .filter(a => validPhoneLogins.has(a.phoneLogin))
      .map(a => {

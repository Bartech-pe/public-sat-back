...
import { BufferedMessage, MessageBufferService } from './message-buffer.service';
import { Channels, CitizenDocType } from '@common/interfaces/multi-channel-chat/channel-message/channel-chat-message.dto';
import { Citizen } from '../entities/citizen.entity';
import { AssistanceStatus } from '../entities/assistance.entity';
...
export class BasicInfoService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => AssistanceService))
    ...
    this.redis.config('SET', 'notify-keyspace-events', 'Ex').then(() => {
      ...
      this.subscriber.on('message', (channel, key) => {
        if (channel === '__keyevent@0__:expired' && key.startsWith('basic_info_timeout:')) {
  ...
  private buildBaseOutgoing(message: BufferedMessage): Omit<OutgoingPayload, 'options'> {
    return {
  ...
  private async createChannelMessageFromBuffered(message: BufferedMessage, content: string) {
    const newMessageSaved = await this.multiChannelChatService.createChannelMessage({
      assistanceId: message.assistance.id,
      channelRoomId: message.channelRoom.dataValues?.id || message.channelRoom.id,
      content,
      senderType: 'bot',
      status: 'unread',
      timestamp: new Date(),
      userId: message.channelRoom.dataValues?.userId || (message.channelRoom.userId as number),
    ...
    let countUnreadMessages = await this.channelMessageRepository.findAndCountAll(
      {
        where: {channelRoomId: message.channelRoom.id, status: 'unread', senderType: 'citizen'}
      }
    )
    let newMessage: ChannelRoomNewMessageDto = {
      ...
      message: {
        ...
        status: 'unread',
        attachments: [],
        time: new Date(newMessageSaved.dataValues.timestamp).toLocaleTimeString('es-PE', {
          hour: '2-digit',
          minute: '2-digit',
        }),
        fromMe: true,
  ...
  private async maybeSendGreetings(message: BufferedMessage, includeGreeting: boolean) {
    if (!includeGreeting) return;
    const greetings = ['Bienvenido al *SAT DE LIMA*', 'Hola! Soy su asesor virtual 🤖'];
  ...
  private async setStepAndTimeout(phone: string, step: 'name' | 'documentType' | 'documentNumber' | 'confirmExistingData') {
  ...
  private hasCompleteData(citizen: Citizen): boolean {
    return !!(citizen.fullName && citizen.documentType && citizen.documentNumber);
  }

  public async handleBasicInfoMessage(message: BufferedMessage): Promise<{ handled: boolean }> {
    ...
    try {
      ...
      await this.redis.set(`buffered_message:${phone}`, JSON.stringify(message), 'EX', 650);
      ...
      this.logger.debug(`[${phone}] Estado: inQueue=${inQueue}, hasGreeted=${hasGreeted}, step=${step}`);

      if (!inQueue) {
        await this.redis.set(`user_state:${phone}`, 'requestingBasicInformation');
  ...
  private resetBufferTimer(phone: string) {
    this.logger.debug(`[${phone}] Reseteando timer del buffer (${this.bufferWaitTime}ms)`);
    ...
    const timeout = setTimeout(async () => {
      try {
        this.logger.debug(`[${phone}] Timer expirado, procesando buffer de nombres`);
  ...
  private async processNameBuffer(phone: string) {
    this.logger.debug(`[${phone}] Iniciando procesamiento del buffer de nombres`);
    ...
    if (jsonMsgs.length === 0) {
      await this.redis.set(`debug:${phone}:buffer_empty`, `${new Date().toISOString()} - Buffer was empty`, 'EX', 3600);
    ...
    const combinedBody = buffered.map((m) => m.data.payload.message.body).join(' ').trim();
    ...
    if (result.success) {
      this.logger.debug(`[${phone}] Nombre procesado exitosamente, solicitando tipo de documento`);
  ...
  private async processDataConfirmation(body: string) {
    ...
    } else if (['no', 'n', 'incorrecto', 'false', 'falso', 'nope'].includes(cleanedBody)) {
      return { confirmed: false, message: '' };
    } else {
      return { confirmed: null, message: 'Por favor responda *Sí* o *No* para confirmar si los datos son correctos.' };
    }
  }

  private async sendMessageNotUnderstoodStatus(message: BufferedMessage, reason: string) {
    message.data.payload.message.body = reason;
    await this.sendAndPersist(message, { type: 'text', text: reason });
  }

  private async processName(citizen: Citizen, body: string) {
    const regex = /^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]+$/;
    const words = body.trim().split(/\s+/).filter(word => word.length > 0);
    if (regex.test(body) && words.length >= 2 && words.every(word => word.length >= 2)) {
      await this.citizenRepository.update(citizen.id, { fullName: body.trim() });
      citizen.fullName = body.trim();
      return { success: true, message: '' };
    } else {
      return { success: false, message: 'Por favor, ingrese correctamente sus *nombres y apellidos completos* (mínimo 2 palabras)' };
    }
  }

  private async processDocumentType(citizen: Citizen, body: string) {
    const cleanedBody = body.trim().toUpperCase();
    if (['DNI', 'CE', 'OTROS', 'OTRO'].includes(cleanedBody)) {
      await this.citizenRepository.update(citizen.id, { documentType: cleanedBody as CitizenDocType });
      citizen.documentType = cleanedBody as CitizenDocType;
      return { success: true, message: '' };
    } else {
      return { success: false, message: 'Por favor, seleccione una opción válida: *DNI*, *CE* o *OTROS*' };
    }
  }

  private async processDocumentNumber(citizen: Citizen, body: string) {
    ...
    const docType = (currentCitizen?.dataValues?.documentType || '').toUpperCase();
    ...
    if (regex.test(digitsOnly)) {
      await this.citizenRepository.update(citizen.id, { documentNumber: digitsOnly });
      citizen.documentNumber = digitsOnly;
      return { success: true, message: '' };
    }

    const msg = docType === 'CE'
  ...
  private async requestDataConfirmation(message: BufferedMessage, includeGreeting: boolean = true) {
    const citizen = message.citizen;
    ...
    message.data.payload.message.body = summary + `\n*${footerText}*\n` + `- Si\n - No`
    ...
    await this.setStepAndTimeout(message.citizen.phoneNumber!, 'confirmExistingData');
  }

  private async requestNewBasicInformation(message: BufferedMessage, includeGreeting: boolean = true) {
  ...
  private async requestName(message: BufferedMessage, includeGreeting: boolean = true) {
    await this.maybeSendGreetings(message, includeGreeting);
    const ask = 'Por favor para una mejor atención, ingrese sus *nombres y apellidos completos*';
  ...
  private async requestDocumentType(message: BufferedMessage, includeGreeting: boolean = false) {
    this.logger.debug(`[${message.citizen.phoneNumber}] Solicitando tipo de documento`);
    ...
    this.logger.debug(`[${message.citizen.phoneNumber}] Tipo de documento solicitado, step actualizado`);
  }

  private async requestDocumentNumber(message: BufferedMessage) {
    const txt = 'Ahora, escriba *el número de su documento* 🪪\nEjemplo: 12345678';
    message.data.payload.message.body = txt;
    await this.sendAndPersist(message, { type: 'text', text: txt });
    await this.setStepAndTimeout(message.citizen.phoneNumber!, 'documentNumber');
  }
  ...
  public async onBasicInfoTimeout(phone: string) {
    const bufferedMessageStr = await this.redis.get(`buffered_message:${phone}`);
    ...
    const assistance = await this.assistanceRepository.findById(bufferedMessage.assistance.id)
    if(assistance.status !== AssistanceStatus.CLOSED)
    {
      const closing = 'Por tu seguridad estamos cerrando la conversación🔐.\nPuedes volver a escribirnos cuando desees, estamos para servirte 👋😄';
      bufferedMessage.data.payload.message.body = closing;
      await this.sendAndPersist(bufferedMessage, { type: 'text', text: closing });
  
      await this.assistanceService.closeAssistance({
    ...
    try {
      ...
      for (const key of keysToDelete) {
        try {
          ...
          this.logger.warn(`[${phone}] Error eliminando clave ${key}:`, keyError);
        }
      }
    }
  }

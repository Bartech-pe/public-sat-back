...
import { Assistance, AssistanceAttributes } from '../entities/assistance.entity';
...
import { AdvisorAssigned, ChannelRoomNewMessageDto } from '@common/interfaces/multi-channel-chat/channel-room/channel-room-summary.dto';
...
export interface BufferedMessage {
  ...
  assistance: AssistanceAttributes;
  citizen: Citizen; 
...
export class MessageBufferService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly waitTime = 8000;
  ...
  async onModuleInit() {
    this.logger.log('MessageBufferService iniciado con buffer dinámico y timeout');
    
    await this.redisClient.config('SET', 'notify-keyspace-events', 'Ex');
    
    await this.subscriber.subscribe('__keyevent@0__:expired', (err) => {
      if (err) {
        this.logger.error('Error al suscribirse a eventos de Redis para timeout', err);
      }
    });

    this.subscriber.on('message', (channel, key) => {
      if (channel === '__keyevent@0__:expired' && key.startsWith('message_buffer_timeout:')) {
  ...
  async addMessageToBuffer(message: BufferedMessage) {
    try {
      ...
      const botMustAnswer = ( [ChannelType.WHATSAPP, ChannelType.CHATSAT].includes(message.data.payload.channel) && message.channelRoom.botReplies);


      if(botMustAnswer)
      {
          await this.redisClient.rpush(this.bufferKey, JSON.stringify(message));
          this.logger.debug(`Mensaje agregado al buffer: ${message.data.payload.message.body}`);
          await this.redisClient.set(`buffered_message:${senderId}`, JSON.stringify(message), 'EX', 650);
      }
  ...
  private async setMessageBufferTimeout(senderId: string) {
    if (!senderId) return;
    await this.redisClient.set(`message_buffer_timeout:${senderId}`, '1', 'EX', this.timeoutDuration);
  ...
  private async onMessageBufferTimeout(senderId: string) {
    try {
      ...
      const bufferedMessageStr = await this.redisClient.get(`buffered_message:${senderId}`);
      ...
      if(assistance.status !== AssistanceStatus.CLOSED)
      {
         const outMessage: OutgoingPayload = {
          ...
          await this.createChannelMessageFromBuffered(bufferedMessage, "Por tu seguridad estamos cerrando la conversación🔐.\nPuedes volver a escribirnos cuando desees, estamos para servirte 👋😄");
      ...
      await this.cleanupMessageBufferProcess(senderId);
      this.logger.log(`Conversación cerrada por timeout para usuario: ${senderId}`);
      
    } catch (error) {
      this.logger.error(`Error procesando timeout para usuario ${senderId}:`, error);
      await this.cleanupMessageBufferProcess(senderId);
    }
  }
  ...
  private async cleanupMessageBufferProcess(senderId: string) {
    try {
      ...
      this.logger.error(`Error en cleanup Message Buffer para ${senderId}:`, error);
    }
  }
    ...
    try {
      ...
      const filteredMessages = messages.filter(messageStr => {
        try {
          ...
          const msgPhoneNumber = (message?.citizen?.phoneNumber ?? '').toString();
      ...
      this.logger.debug(`Buffer cleanup: ${removedCount} mensajes removidos para usuario ${senderId}`);
      
    } catch (error) {
      this.logger.error(`Error en cleanup de buffer compartido para ${senderId}:`, error);
      await this.redisClient.unwatch();
    }
  }
  ...
  private resetTimer(processMessage: boolean) {
    ...
    this.logger.debug(`Temporizador reiniciado, procesará en ${this.waitTime}ms`);
  }
  ...
  private async processBufferedMessages() {
    try {
      ...
      const bufferedMessages: BufferedMessage[] = messages.map(m => JSON.parse(m));
      this.logger.log(`Procesando batch de ${bufferedMessages.length} mensajes`);

      const grouped = bufferedMessages.reduce((acc, msg) => {
      ...
      const timeoutKeys = Object.keys(grouped).map(senderId => `message_buffer_timeout:${senderId}`);
      if (timeoutKeys.length > 0) {
        try {
          ...
          this.logger.warn('Error limpiando timeouts en batch, continuando:', error);
        }
      }
      for (const senderId in grouped) {
        const combinedMessage = grouped[senderId].map(m => m.data.payload.message.body).join(' ');
        ...
        try {
          const responses = await this.rasaService.sendMessageToRasa(rasaPayload);
          for (const response of responses) {
            const outMessage: OutgoingPayload = {
              channel: exampleMsg.data.payload.channel as Channels,
              botReply: true,
              citizenId: exampleMsg.channelRoom.citizenId,
            ...
            // RESTAURADO: Usar método específico con gateway
            await this.createChannelMessageFromBuffered(exampleMsg, response.text?.replaceAll('**', '*') ?? '');
          }

          // RESTAURADO: Renovar timeout después de procesar
          await this.redisClient.set(`buffered_message:${senderId}`, JSON.stringify(exampleMsg), 'EX', 650);
          await this.setMessageBufferTimeout(senderId);
        } catch (err) {
          this.logger.error(`Error enviando mensaje a Rasa para sender ${senderId}`, err);
        }
      }
    } catch (error) {
  ...
  private async createChannelMessageFromBuffered(message: BufferedMessage, content: string) {
    const newMessageSaved = await this.multiChannelChatService.createChannelMessage({
      ...
      channelRoomId: message.channelRoom.dataValues?.id || message.channelRoom.id, // CORREGIDO: Manejo robusto del ID
      content,
      senderType: 'bot',
      status: 'unread',
      timestamp: new Date(),
  ...
  public async hasActiveTimeout(senderId: string): Promise<boolean> {
    const exists = await this.redisClient.exists(`message_buffer_timeout:${senderId}`);
    return exists === 1;
  }

...
import { ChannelRoomNewMessageDto, ChannelRoomSummaryDto, ChannelRoomViewStatusDto } from '@common/interfaces/multi-channel-chat/channel-room/channel-room-summary.dto';
...
export class MultiChannelChatGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect{
  @WebSocketServer()
  server: Server;
  ...
  async handleConnection(client: any) {
     try {
        ...
        if (token) {
          ...
          const user = await this.userRepository.findById(payload.sub, { include: [{ model: Role }] });
          client.user = user;
        }
      } catch (err) {
  ...
  notifyAdvisorRequest(channelRoomId: number, assistanceId: number, userId: number)
  {
  ...
  handleTypingIndicator(
    @MessageBody() payload: ChannelRoomAssistance
  ) 
  {

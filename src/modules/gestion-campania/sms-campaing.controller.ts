...
export class SmsCampaingController {
    constructor(
        private readonly smsCampaingService:SmsCampaingService
    ){}
    @Post('readSMSExcel')
    ...
    async previewMessage(@Body() body: MessagePreview) {
        return await this.smsCampaingService.getMessages(body.rows, body.message, body.contact)
    }
    @Post()
    ...
    async sendMessages(@Body() body: MessagePreview){
        return await this.smsCampaingService.buildMessages(body.rows,body.message,body.contact)
    }
}

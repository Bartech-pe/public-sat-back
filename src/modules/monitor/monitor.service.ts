...
export class MonitorService {
  ...
  async channelCount(channelId: number) {
    ...
    const result = await this.attentionRepository.findAll({
      where: {
        createdAt: {
          [Op.gte]: today,
          [Op.lt]: tomorrow,
    ...
    const allAttentionsToday = result.map((r) => r.toJSON());

    const total = allAttentionsToday.length;

    const complete = allAttentionsToday.filter(
      (a) => a.status === ChannelAttentionStatus.CLOSED,
    ).length;

    const inComplete = allAttentionsToday.filter(
      (a) => a.status === ChannelAttentionStatus.IN_PROGRESS,
    ).length;

    const inQueque = allAttentionsToday.filter(
      (a) => a.status === ChannelAttentionStatus.IDENTITY_VERIFICATION,
    ).length;
  ...
  async getMonitoringMultiChannel(channelId: number) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const tomorrow = new Date(today);
    tomorrow.setDate(today.getDate() + 1);

...
import { MailAttentionRepository } from "@modules/gmail/repositories/mail-attention.repository";
...
import { AssistanceRepository } from "@modules/multi-channel-chat/repositories/assistance.repository";
...
import { UserVicidialRepository } from "@modules/user/repositories/user-vicidial.repository";
...
export class MonitorService {
    constructor(private readonly assitanceRepository:AssistanceRepository,
    ...
    private readonly userVicidialRepository:UserVicidialRepository,
    ...
    async channelBotCount(channelId:number){
        ...
        const assitances = await this.assitanceRepository.findAll({
        ...
        const messages = await this.channelMessageRepository.findAll({
            ...
            attributes: [
                ...
                [
                    this.sequelize.fn(
                        'MAX',
                        this.sequelize.literal(`CASE WHEN senderType = 'bot' THEN 1 ELSE 0 END`)
                    ),
                    'hasBot',
                ],
                [
                    this.sequelize.fn(
                        'MAX',
                        this.sequelize.literal(`CASE WHEN senderType = 'agent' THEN 1 ELSE 0 END`)
                    ),
                    'hasAgent',
                ],
            ],
    ...
    async channelCount(channelId:number){
        ...
        const total = await this.assitanceRepository.count({
            where: { createdAt: {
                    [Op.gte]: today,
                    [Op.lt]: tomorrow,
                } } as any,
        ...
        const complete = await this.assitanceRepository.count({
            where: { status: AssistanceStatus.CLOSED, createdAt: {
                    [Op.gte]: today,
                    [Op.lt]: tomorrow,
                } } as any,
            include: [baseInclude],
        ...
        const inComplete = await this.assitanceRepository.count({
            where: { status: AssistanceStatus.IN_PROGRESS, createdAt: {
                    [Op.gte]: today,
                    [Op.lt]: tomorrow,
                } } as any,
            include: [baseInclude],
        });
        const inQueque = await this.assitanceRepository.count({
            where: { status: AssistanceStatus.IDENTITY_VERIFICATION, createdAt: {
                    [Op.gte]: today,
                    [Op.lt]: tomorrow,
                } } as any,
            include: [baseInclude],
    ...
    async totalChat(userId:number,channelId:number,today:Date,tomorrow:Date){
         const totalChat = await this.assitanceRepository.count({
            where: { createdAt: {
                    [Op.gte]: today,
                    [Op.lt]: tomorrow,
                } } as any,
    ...
    async attentionDetail(userId:number){
        ...
        const recievedCount = await  this.mailAttentionRepository.count({
            where: { advisorUserId:userId,createdAt: {
                    [Op.gte]: today,
                    [Op.lt]: tomorrow,
                }, } as any,
        });
        ...
        let alosat:number = 0;
        const vicidialuser = await this.userVicidialRepository.findOne({ where: { idUser: userId } })
        if (vicidialuser) {
    ...
    async mailCount() {
        ...
        return {
            attendedCount,notAttendedToday,notAttendedYesterday,recievedCount
        }
    }
    ...
    async getAdvisors(channelId:number){
        const advisors = await this.inboxUserRepository.findAll({
            include: [{ model: User,attributes:['displayName'], include: [{ model: Role, where: { id: 3 },attributes:[] }] },
            {
                model: Inbox,attributes:[] ,include: [{
                    model: Channel,
                    where: { id: channelId },attributes:[]
                }]
            },{
                model:EstadoCanal,
                attributes: ['nombre']
            ...
            attributes: ['idUser', 'updatedAt','idInbox']
        });
    ...
    async getMailAdvisors() {
        ...
        const totalAttentions:number = (Object.values(attentionMap) as number[]).reduce(
            (sum: number, count: number) => sum + count,
            0,
        );
        const advisorsResult = advisors.map((item) => {
            const attentionCount = attentionMap[item.idUser] || 0;
    ...
    async getChatAdvisors() {
        ...
        const assistanceGrouped = await this.assitanceRepository.findAll({
            attributes: [
                [this.sequelize.col('channelRoom.userId'), 'userId'],
                [this.sequelize.fn('COUNT', this.sequelize.col('Assistance.id')), 'count'],
                [
                    this.sequelize.fn(
                        'AVG',
                        this.sequelize.literal(
                            'TIMESTAMPDIFF(SECOND, `Assistance`.`startDate`, `Assistance`.`endDate`)'
                        )
                    ),
                    'avgDuration',
                ],
        ...
        const asistanceGroupedJson = assistanceGrouped.map((a) => a.toJSON() as any)
        ...
        const totalAttentions: number = (Object.values(assistanceMap) as { count: number; avgDuration: number }[]).reduce(
            (sum, item) => sum + item.count,
            0,
        );
        const advisorsResult = advisors.map((item) => {
            const attentionCount = assistanceMap[item.idUser] || 0;
            const updatedAtValue = item.updatedAt ?? 0;
            const diffMinutes = Math.floor(
                (Date.now() - new Date(updatedAtValue).getTime()) / (1000 * 60),
            );

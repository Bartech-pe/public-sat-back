...
import {
  UpdateVicidialCallTime,
  VicidialCallTimeBody,
} from '../dto/call-time/update-vicidial-call-time.dto';
import { VicidialCallTimesHolidays } from '../entities/vicidial-call-times-holidays.entity';
import { Op } from 'sequelize';
import {
  HolidayCallTime,
  VicidialCallTimeDto,
} from '../dto/call-time/vicidial-call-time.dto';
...
export class VicidialCallTimesService {
  constructor(
    private readonly repo: VicidialCallTimesRepository,
    private readonly repoHolidays: VicidialCallTimesHolidaysRepository,
  ) {}

  async UpdateCallTime(call_time_id: string, body: UpdateVicidialCallTime) {
    try {
      const exist = await this.repo.getModel()!.findOne({
        where: { call_time_id },
      });
      if (!exist) {
      ...
      if (body.everyday) {
        const evetydayBody: VicidialCallTimeBody = {
          ...
          call_time_comments:
            body.call_time_name ?? callTime.call_time_comments,
        ...
        const selectedBody: VicidialCallTimeBody = {
          ...
          ct_wednesday_start:
            body.wednesdayStart ?? callTime.ct_wednesday_start,
  ...
  async GetCallTime(call_time_id: string) {
    ...
    const callTime = exist.toJSON();
    const findHolidays: string[] = callTime.ct_holidays
      .split('|')
      .filter(Boolean);
    const existHolidays = await this.repoHolidays.getModel()!.findAll({
      where: {
        holiday_id: {
          [Op.in]: findHolidays,
        },

...
import { CreateSmsCampaignDetail } from '../dto/create-sms-campaign-detail.dto';
import * as XLSX from 'xlsx';
import { UpdateSmsCampaignDetail } from '../dto/update-sms-campaign-detail.dto';
...
@Injectable()
export class SmsCampaignDetailService {
  constructor(
    private readonly campaignTypeRepository: CampaignTypeRepository,
    private readonly SmsCampaignDetailRepository: SmsCampaignDetailRepository,
    private readonly srvmensajeriaService: SrvmensajeriaService,
  ) {}
  async findAll(limit: number, offset: number) {
    const data = await this.SmsCampaignDetailRepository.findAndCountAll({
      limit,
      offset,
      order: [['createdAt', 'ASC']],
      attributes: [
  ...
  async findOne(id: number) {
    try {
      const exist = await this.SmsCampaignDetailRepository.findOne({
        where: { id: id },
        // include: [
        //   {
        //     model: Campaign,
  ...
  async update(id: number, body: UpdateSmsCampaignDetail) {
    const exist = await this.SmsCampaignDetailRepository.findById(id);
    if (!exist) {
      throw new NotFoundException('Campaña no encontrado');
    }
  ...
  remove(id: number): Promise<void> {
    try {
      return this.SmsCampaignDetailRepository.delete(id);
    } catch (error) {
      throw new InternalServerErrorException(
        error,
        'Error interno del servidor',
  ...
  async createSmsCampaignDetail(body: CreateSmsCampaignDetail) {
    const type = await this.campaignTypeRepository.getSMS();
    ...
    const sms = await this.SmsCampaignDetailRepository.create(createsms);
